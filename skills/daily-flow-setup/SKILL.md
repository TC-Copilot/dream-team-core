...
This setup skill targets the stable public core release **4.5.33**. Its build revision is
**20260916.1**. Keep those identities separate:

- Use `4.5.33` as the semantic release identity for compatibility decisions.
- `buildRevision = 20260916.1` and the release asset SHA-256 are artifact identities only. They may
  trigger replacement files when upgrading to `4.5.33`, but they are not
...
- If `app\.version-report.json` exists, confirm its core version is `4.5.33` before continuing and
