...
This setup skill targets the stable public core release **4.5.34**. Its build revision is
**20260916.2**. Keep those identities separate:

- Use `4.5.34` as the semantic release identity for compatibility decisions.
- `buildRevision = 20260916.2` and the release asset SHA-256 are artifact identities only. They may
  trigger replacement files when upgrading to `4.5.34`, but they are not
...
- If `app\.version-report.json` exists, confirm its core version is `4.5.34` before continuing and
