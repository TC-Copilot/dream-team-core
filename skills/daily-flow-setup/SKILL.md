...
1. **Read the currently running version.** Call `GET http://127.0.0.1:<port>/api/state` first to confirm the app answers, then `GET http://127.0.0.1:<port>/api/health` and read `.version`. Call this `INSTALLED_VERSION`. `buildRevision`, when present, is diagnostic only: never treat it as a newer product release or include it in the release-version comparison.
