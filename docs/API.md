...
```json
{ "ok": true, "version": "4.5.20", "coreVersion": "4.5.20", "buildRevision": "20260818.6", "serverTime": "2026-08-18T15:40:26Z" }
```
...
`title` and `format` are required. `format` is `docx`, `text`, or `markdown`, and non-empty
`content` is required. PowerPoint is intentionally not generated by this endpoint: workers invoke
Scout's built-in `pptx` skill, save a genuine `.pptx` under the configured document root, and report
that local path through `POST /api/jobs/{jobId}`. The server rejects legacy `.ppt`, manually
assembled OpenXML/ZIP, renamed HTML/text, and malformed packages instead of returning a
repair-required presentation.
