...
This setup skill targets the stable public core release **4.5.24**. Its build revision is
**20260820.2**. Keep those identities separate:

- Use `4.5.24` as the semantic release identity for compatibility decisions.
- `buildRevision = 20260820.2` and the release asset SHA-256 are artifact identities only. They may
  trigger replacement files when upgrading to `4.5.24`, but they are not
...
- If `app\.version-report.json` exists, confirm its core version is `4.5.24` before continuing and
