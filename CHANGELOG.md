...
- Two independent checkboxes above the "Results and drafts prepared" section on the main dashboard and above the calendar on `results-history.html`: **"Hide company names"** and **"Hide person names"**. Both are off by default, each choice is remembered separately per browser (`localStorage`, shared between the two pages), and toggling either off immediately restores the real names on the next render — nothing is ever sent to the server or mutated in stored data.
